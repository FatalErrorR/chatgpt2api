        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              className="h-10 rounded-xl border-stone-200 bg-white"
            />
            <p className="text-xs text-stone-500">仅 gpt-image-2 会使用该上游模型名（默认 gpt-5-5）。gpt-image-2.5 / flare / sunburst 会原样发给 ChatGPT。</p>
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-stone-500">Web 生图参考图整包超过 1.4MB 时自动缩边/降质量，避免会话 413。关闭后按原图上传。</p>
          </div>
          <div className="space-y-2">
            <div className="flex items-center gap-3 rounded-xl border border-stone-200 bg-white px-4 py-3">
              <Checkbox
                checked={Boolean(config?.image_req_fit_enabled !== false)}
                onCheckedChange={(checked) => setImageReqFitEnabled(Boolean(checked))}
              />
              <span className="text-sm text-stone-700">请求超预算压缩</span>
            </div>
            <p className="text-xs text-stone-500">提示词 + 参考图超过 1.4MB 时只压参考图，提示词原文不动。与上一开关独立。</p>
          </div>
          <div className="space-y-2">
            <div className="flex items-center gap-3 rounded-xl border border-stone-200 bg-white px-4 py-3">
              <Checkbox
